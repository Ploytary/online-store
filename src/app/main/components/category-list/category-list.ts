...
export class CategoryList extends BaseComponent {
  ...
  constructor(props: ComponentProps = {}) {
    ...
    const listTitleElement = new BaseComponent({ tagName: 'h4', classList: ['section__title'], content: 'Category' })
      .node;
    const expandLink = new BaseComponent({
      tagName: 'a',
      classList: ['category-list-section__expand-link', 'link'],
      content: 'show more',
    }).node;
  ...
  getItems(list: string[], listElement: HTMLElement): void {
    ...
    list.forEach((categoryName) => {
      const listItem = new BaseComponent({
        tagName: 'li',
        classList: ['category-list-section__list-item', 'section__list-item'],
      }).node;
  ...
  toggleExpand(): void {
    ...
    this.expandButton.addEventListener('click', (event: Event) => {
      ...
      if (event.target instanceof HTMLElement) {
        ...
        this.getListStateClass();
        this.state.closed
          ? (this.expandButton.textContent = 'show more')
