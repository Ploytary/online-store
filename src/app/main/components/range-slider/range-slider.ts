...
export class RangeSlider extends BaseComponent {
  ...
  constructor(props: ComponentProps = {}, initMinValue = 0, initMaxValue = 1000) {
    ...
    this.minValueInputElement = new Input({
      classList: ['range-slider__min'],
      type: 'number',
      value: this.minValue.toString(),
    }).node as HTMLInputElement;
    this.maxValueInputElement = new Input({
      classList: ['range-slider__max'],
      type: 'number',
    ...
    this.handlerMinElement = new Input({
      classList: ['range-slider__handler-min', 'range-slider__handler'],
      type: 'range',
      min: '0',
      max: maxSliderRangeValue.toString(),
      step: '1',
      value: this.minValue.toString(),
    }).node as HTMLInputElement;
    this.handlerMaxElement = new Input({
      classList: ['range-max', 'range-slider__handler'],
      type: 'range',
      min: '0',
      max: maxSliderRangeValue.toString(),
      step: '1',
    ...
    priceInput.forEach((input) => {
      input.addEventListener('input', (evt: Event) => {
        const minVal = parseInt(this.minValueInputElement.value);
        const maxVal = parseInt(this.maxValueInputElement.value);
        if (evt.target === null) {
          return;
        }
    ...
    rangeInput.forEach((input) => {
      input.addEventListener('input', (evt: Event) => {
        const minVal = parseInt(this.handlerMinElement.value);
        const maxVal = parseInt(this.handlerMaxElement.value);
        if (evt.target === null) {
          return;
        }
        if (!(evt.target instanceof HTMLElement)) {
