...
export class Rating extends BaseComponent {
  ...
  constructor(props: ComponentProps) {
    ...
    const starClassArray = new Array(5)
      .fill('star-empty')
      .map((star, index) => (index < Math.round(+value) ? 'star-filled' : star));
    ...
    const ratingNumericValue = new BaseComponent({
      tagName: 'p',
      classList: ['rating__value'],
