...
export class ProductCardPreview extends BaseComponent {
  ...
  constructor(productData: any, props: ComponentProps = {}) {
    ...
    const descriptionComponent = new BaseComponent({tagName: 'p', classList: ['product-card-preview__description'], content: this.description});
    const priceComponent = new BaseComponent({tagName: 'p', classList: ['product-card-preview__price']});
    const basePrice = new BaseComponent({tagName: 'span', classList: ['product-card-preview__base-price'], content: '€' + this.price.toString()})
    const discountPrice = new BaseComponent({tagName: 'span', classList: ['product-card-preview__discount-price'], content: '€' + this.discountPrice.toString()})
    priceComponent.node.append(discountPrice.node, basePrice.node);
    const stockComponent = new BaseComponent({tagName: 'p', classList: ['product-card-preview__stock'], content: 'In stock: ' + this.stock.toString()});
    const ratingComponent = new Rating({value: this.rating, classList: ['product-card-preview__rating']});
    const addCartButtonComponent = new Button({content: 'Add to cart', classList: ['product-card-preview__cart-button', 'button--accent']})
    this.node.append(imageComponent.node, titleComponent.node, brandComponent.node, categoryComponent.node, descriptionComponent.node, priceComponent.node, ratingComponent.node, addCartButtonComponent.node, stockComponent.node);
  }
}